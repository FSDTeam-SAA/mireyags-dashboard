...
import { CategoriesApiResponse } from "@/app/(dashboard)/category-management/_components/category-all-data-type";
import { BrandsApiResponse } from "@/app/(dashboard)/brand-management/_components/brands-data-type";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function EditProductForm({ id }: { id: string }) {
  ...
  React.useEffect(() => {
    ...
    form.reset({
      ...
      category: getIdValue(product.category?.name),
      brand: getIdValue(product.brand?.name),
  ...
  >({
    ...
    onSuccess: async (data) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["single-product", id] });
    },
    onError: (error) => {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      className="w-full h-[48px] text-base text-[#1E1E1E] leading-normal font-medium border border-[#CECECE] rounded-[8px] placeholder:text-[#CECECE]"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className="h-[48px] w-full rounded-[8px] border border-[#CECECE] text-base font-medium text-[#1E1E1E]">
                        <SelectValue
                          placeholder={
                            isCategoryLoading
                              ? "Loading categories..."
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-medium text-[#1E1E1E] leading-normal pb-3">
                Main Image
              </h3>
            ...
            <div>
              <div className="mb-3 flex items-center justify-between gap-3">
                <h3 className="text-lg font-medium text-[#1E1E1E] leading-normal">
                  Sub Images
                </h3>
